...
const calculateTailHead = (input) => {
    ...
    while(stack.length > 0) {
       //total += calculateTailHeadHelper(input, stack)
        const {elevation, position:{row, column}} = stack.pop()
        if(elevation === 0) {
            input = copyInput(savedInput)
        }
        if(elevation === 9) {
            input[row][column] = '.'
            total++
...
const calculateTailHeadHelper = (input, stack, total = 0) => {
    if(stack.length === 0) {
        return total
    }
    ...
    return calculateTailHeadHelper(input, stack, total)
}
